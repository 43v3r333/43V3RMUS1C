...
class TestDistributedScenarios:
    ...
    async def test_distributed_counter_consistency(self, node_simulator: DistributedNodeSimulator):
        ...
        initial_messages = node_simulator._metrics.total_messages
        
        # Simulate counter increments via broadcast
        for i in range(10):
            await node_simulator.broadcast("counter-node-1", "increment", {"value": i})
        
        # Broadcast should have sent messages to neighbors
        # Each broadcast sends to 1 neighbor (counter-node-2)
        assert node_simulator._metrics.total_messages > initial_messages
        # All nodes should exist and be active
        for node_id in ["counter-node-1", "counter-node-2", "counter-node-3"]:
            assert node_simulator.nodes[node_id].state == NodeState.ACTIVE
